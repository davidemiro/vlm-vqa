...
def main():
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="steps",
        eval_steps=2, #len(dataset_train) // (int(config['batch_size']) * int(config['gradient_accumulation_steps']) * int(config['num_gpu'])),
        save_steps=2, #len(dataset_train) // (int(config['batch_size']) * int(config['gradient_accumulation_steps']) * int(config['num_gpu'])),
